...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    addToCart: (state, action) => {
      ...
      const itemExists = state.userCart?.products?.find((item) => item.product_id === action.payload.product.id);
        if (itemExists) {
          itemExists.quantity++;
          alert("item quantity increase!!");
        } else {
          state.userCart.products.push({ ...action.payload.product, quantity: 1 });
    ...
    incrementQuantity: (state, action) => {
      const item = state.userCart.products.find((item) => item.product_id === action.payload);
      item.quantity++;
    },
